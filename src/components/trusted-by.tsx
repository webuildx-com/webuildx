  ...
  return (
    ...
    >
      <motion.div
        ...
        variants={fadeUpSafe}
      >
        <p className="shrink-0 text-[10px] font-medium uppercase leading-none tracking-[0.2em] text-subtle">
      ...
      <motion.div
        className="grid grid-cols-3 gap-y-4 sm:flex sm:gap-y-0"
        variants={staggerDelayed}
      >
        {logos.map(({ id, label }, index) => (
          <motion.div
            key={id}
            className="relative flex min-w-0 flex-1 items-center justify-center px-2 py-2 sm:px-3 sm:py-3"
            variants={fadeUpSafe}
          >
            {index > 0 && (
              <span
                className="absolute left-0 top-1/2 hidden h-8 w-px -translate-y-1/2 bg-border sm:block"
                aria-hidden="true"
              />
            )}
            <span className="text-center text-[10px] font-semibold leading-none tracking-tight text-ink/60 sm:text-[12px]">
              {label}
            </span>
          </motion.div>
        ))}
